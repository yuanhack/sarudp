...
>sarudp 在 UDP 基础上实现了请求重传和应答重传。

　　sarudp 同时具有 UDP 和 TCP 的优点，可应用于各种模式的开发，并且可同时支持 IPv6 和 IPv4 应用程序的开发。sarudp 在收到请求包后，在处理函数中可提取出对方的 IP和Port ， 编程时标识和记忆 IP和Port 即可知道对方的身份，使用 sarudp 的 su_serv_t 创建的服务，可以应对任意数量的 su_peer_t 客户端。

>sarudp 没有占用大量文件描述符的问题，不需要担心处理的终端上限。
...
　　sarudp 的 send 函数向对方发送数据然后返回，就像普通的UDP数据 sendto 那样。<br>
...
* **为减小篇幅以及便于描述，本文约定：**
	* 若非特指，`request，send, create 或 reply` 指 `su_peer_t 或 su_serv_t` 相应的等函数；
	* `请求`是指主动向对方发送数据，调用`request` 发送数据就是发送请求；
	* `应答`是指在`数据处理函数`中调用 `reply` 函数向`请求`方发送应答；
	* `reliable 数据`是具有传输可靠性的数据，具有重传，缓存和应答机制；
	* `ordinary 数据`是不具有传输可靠性的数据，没有重传，缓存和应答机制；
	* `reliable 函数`是用户为处理收到的 reliable 数据请求而实现的回调函数；
	* `ordinary 函数`是用户为处理收到的 ordinary 数据请求而实现的回调函数；
	* `数据处理函数` 或简称`处理函数`是指 reliable函数 或 ordinary函数，或二者皆有。
...
　　sarudp 对请求的处理采用线程的方式，当收到数据时，检查协议头，如果是合法的 sarudp 包就放入链表，通知线程处理，不是就丢弃。处理线程以回调方式调用`用户安装的处理函数`处理请求。<br>
> 当网络接口有数据到来时候，handle_su_x_recv 被调用来处理收到的数据；这个函数 在 su_x_create 中被设置为 EPOLLIN事件的回调函数。线程 thread_request_handle 处理从接收队列拿到的 sarudp 请求数据包，在 request_handle 中，根据当前处理的包的类型不同，调用 reliable 或 ordinary 处理函数。<br>
>出于性能上的的考虑，数据接收时申请固定大小的包内存，避免碎片和二次拷贝，如果用户安装了数据处理函数，则在调用后自动释放，如果没有安装就直接释放。<br>
...
```
　　无论何种情况丢包，request 都将重新发包，重试 `RTT_MAXNREXMT`次后如仍没有收到应答，返回-1，同时设置 errno 为 ETIMEDOUT 。可以通过调整宏决定包的重发次数和应答数据的缓存时长，在 unprtt.h 中有宏定义<br>
...
　　sarudp 使用应答超时重发请求实现请求重传，使用缓存应答和再次收到请求时自动重传应答实现应答重传。
　　无论是本地请求的重传还是对方应答的重传都由本地调用 request 引起，当 request 发出请求后，最理想的情况是对方收到请求后尽快发回一个应答，本地 request 得到了这个应答；如果请求或者应答在网络中丢失，request 在超时期间没有收到应答将重发请求，对方收到重传的请求后如果发现已经缓存了应答，就立即从缓存将应答发出。
...
　　request 发出的请求将由对方安装的reliable处理函数 收到并处理，应答是由在reliable处理函数内调用 reply 发出的。

> 请求重传参考了`《UNIX网络编程卷1：套接字联网API》第三版` 22.5 章节中的算法实现。

　　普通数据没有应答。send 函数用于 ordinary 数据发送，发送的数据将由对方安装的 ordinary处理函数收到，这种数据没有实现应答，在处理函数内调用 reply 函数是无效的，但是可以通过调用 request 或  send 函数向数据源地址发送数据。

<span id="应答重传"><span><br><br>
####应答重传
　　作为接收数据的一端，在数据处理函数中，当调用 reply 函数发送应答后，sarudp 将缓存 reliable 数据，如果应答在中途丢失，再次收到对方的请求后将自动应答 (付出少量内存资源和红黑树查找代价实现应答重传)。<br>
...
###2. 缓存
　　sarudp 在 首次收到请求包 和 首次发送应答包 时，用`红黑树节点`进行数据缓存，同时将该节点加入`循环链表`，链表主要用于过期清理。<br>
　　如果包在传输途中丢失，再次收到同一请求包时将直接在`协议层自动重传应答`，不会再次将请求递送到`数据处理函数`，也不会再次加入缓存。<br>
...
* `注意：`设置的重传次数和间隔的总时长不能超过缓存时间，如果缓存的应答被超时清理，重传的请求将当成新数据被递送到处理函数中处理。一般将其设计为缓存时长的一半，这样的话当 request 请求失败以后可尝试调用 request_retry 进行重试。
...
```
　　`ptoaddr` 是接收方的地址信息，`serveln` 是地址结构的尺寸，`port` 是指定的端口值；<br>
　　`su_peer_create` 创建时随机占用一个的空闲端口，`su_peer_create_bind` 创建时绑定指定的 `port` 端口，如果此端口被占用，创建失败将返回-1，errno 值被设置错误码 `EADDRINUSE`，如果成功，将返回socket描述符的值(>=0)。<br>

　　编译时是否定义了宏 `promiscuous_mode` 将影响 su_peer_t 的数据接收和处理方式。<br>
　　如果不使用宏`promiscuous_mode`，su_peer_t 端将丢弃 ptoaddr 地址之外的任何数据，例如可以以此方式创建端对端的通信应用，两端均使用 su_peer_t 创建 psar 时绑定一个已知的端口，ptoaddr 相互指定对方的通信地址，然后就可以互相收发数据了。<br>
　　如果使用宏 `promiscuous_mode`，psar 安装的数据处理函数将可处理来自任意地址的 sarudp 数据包，但因为给`su_peer_t`仅实现了一个处理线程，因此并不具备太强的处理能力，如要创建专业的服务进程应使用 su_serv_t。<br>
* 宏`promiscuous_mode` 不影响 su_serv_t 端的功能。
...
```
　　在创建时可传入一个大于 0 的值 nthread 指定需要安装的处理线程个数，最终能创建多少处理线程数受进程资源的限制；`saddr` 是 psvr 将要绑定到的服务地址指针，servlen 是其地址长度。<br>
* 创建函数都支持 IPv6 和 IPv4，只要在地址的字段设置的时候设置好即可。由于IPv6兼容IPv4，使用 IPv6 地址创建的服务运行时可同时支持 IPv6 和 IPv4。<br>
...
####3. 安装数据处理函数
　　数据处理函数需要自己先实现如何处理从网络收到的reliable或ordinary数据，再调用下列安装函数进行安装，当收到相应类型数据时，安装的数据处理函数被回调使用。
...
```
　　拆除之前给对应`psar`安装的数据处理函数，一旦拆除之后，将忽略收到的数据。<br>
...
```
　　拆除之前给对应`psvr`安装的数据处理函数，一旦拆除之后，将忽略收到的数据。<br>
...
```
　　向 psar 指定的对方发送数据，request 发送 reliable 请求，send 发送 ordinary 数据。<br>
　　send 函数仅用于向对方发送数据，将数据送入网络接口后立即返回。如果成功，返回值是送入网络接口的数据长度，如果失败，返回-1，并且errno值被设置。<br>
　　request 函数向对方发出请求后等待对方应答返回。outbuff 是将要发送的数据指针，outbytes 是发送的数据长度，inbuff 是用于接收应答的缓冲区，inbytes 告知 request 函数提供的缓冲区尺寸，如果缓冲区过小，将无法容纳应答数据，剩余的应答数据将丢弃。一般来说，缓冲区尺寸使用对方的 `REALDATAMAX` 大小即可，这个值是编译时确定的。
...
```
　reliable_data_in 数据处理函数被安装后，sar 将在收到 reliable 数据时自动调用以进行处理数据。
...
| 程序 | 说明 |
| :---- | :------ |
|su_peer_client | 使用 su_peer_t 实现的程序通常作为客户端使用。|
|su_peer_server | 使用 su_peer_t 实现的程序可提供能力一般的服务。<br>su_peer_t 只有一个数据处理线程，<br>一般用来端对端通讯或处理少量客户端。|
| su_serv_server| 使用 su_serv_t 实现的程序可作专业的服务器程序。<br>su_serv_t 创建时可指定调用数据处理函数的线程并发数。|
