...
int main(int argc, char **argv)
{
    ...
    bindip = "127.0.0.1"; /* if define promiscuous_mode, only receive from 127.0.0.1 */
    struct sockaddr_in servaddr;
    bzero(&servaddr, sizeof(servaddr));
    servaddr.sin_family = AF_INET;
    servaddr.sin_port = htons(65535);
    Inet_pton(AF_INET, bindip, &servaddr.sin_addr);
#else
    bindip = "::1";
    ...
    Inet_pton(AF_INET6, bindip, &servaddr.sin6_addr); // for test localhost IPv6
#endif
    ...
    log_msg("sarudp bind and receive from %s:%s", bindip, argc == 1 ? "10000" : argv[1]);
#endif
